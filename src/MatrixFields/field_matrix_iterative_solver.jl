...
Abstract type for algorithms that approximate a `FieldMatrix` (or something
similar, like a [`LazySchurComplement`](@ref)) with a preconditioner `P` for which
`P * x = b` is inexpensive to solve for `x`. If `P` is a diagonal matrix, then `x`
can be computed as `@. inv(P) * b`; otherwise, the `PreconditionerAlgorithm` must
specify a [`FieldMatrixSolverAlgorithm`](@ref) that can be used to solve
`P * x = b` for `x`. Subtypes: [`MainDiagonalPreconditioner`](@ref),
[`BlockDiagonalPreconditioner`](@ref), [`BlockArrowheadPreconditioner`](@ref),
[`BlockArrowheadSchurComplementPreconditioner`](@ref),
[`WeightedPreconditioner`](@ref), and [`CustomPreconditioner`](@ref).
...
Return a `FieldMatrixSolverAlgorithm` that can be used to solve `P * x = b` for
`x`, where `P` is the preconditioner generated by the `PreconditionerAlgorithm`
`P_alg`. If `P_alg` is `nothing` instead of a `PreconditionerAlgorithm`, or if `P`
is a diagonal matrix (and no solver is required to invert it), this returns
`nothing`.
"""
...
Construct a lazy `FieldMatrix` (or a concrete one when possible) that
...
Allocate and return the cache required to solve the equation `P * x = b`, where
`P` is the preconditioner generated by the `PreconditionerAlgorithm` `P_alg` for
`A`. The cache is an empty `NamedTuple` when `P` is diagonal.
"""
...
Check that `P` is compatible with `b` in the equation `P * x = b`, where `P` is
...
Return the preconditioner `P` from [`lazy_preconditioner`](@ref), materialized
into the concrete `FieldMatrix` `P_cache.P` when the `PreconditionerAlgorithm`
`P_alg` requires a `FieldMatrixSolverAlgorithm` to invert it. Return `nothing` if
`P_alg` is `nothing`.
"""
...
Construct a lazy `FieldVectorView` (or a concrete one when possible) that
represents the product `@. inv(P) * b`. Here, `lazy_b` is a (possibly lazy)
`FieldVectorView` that represents `b`. When `P_alg` requires a solver, the result
is stored in `P_cache.x`.
"""
...
`P` using the `FieldMatrixSolverAlgorithm` `alg`. The default `alg = nothing` is
only permitted when `M` is a diagonal matrix.
"""
...
some initial value `x[0]` (usually the zero vector, ``\\mathbf{0}``) and
...
# Keyword Arguments
  ...
  - `P_alg = nothing`: A `PreconditionerAlgorithm` that specifies how to compute
    `P` and solve `P * x = b` for `x`, or `nothing` if preconditioning is not
    required (in which case `P` is effectively set to `one(A)`).
  - `n_iters = 1`: The number of iterations.
  - `correlated_solves = false`: Whether to set `x[0]` to the value of `x` that
    was generated during the previous call to `field_matrix_solve!`, instead of
    setting it to ``\\mathbf{0}`` (it is always set to ``\\mathbf{0}`` on the first
    call to `field_matrix_solve!`).
  - `eigsolve_kwargs = (;)`: Keyword arguments for the `eigsolve` function that
    ...
    spectral radius).
  - `debug = nothing`: Whether to print debug information with `@debug`. By
    default, `debug` is `true` when the current logger's minimum level is `Debug`,
    e.g., when `"error_norm"` or `"spectral_radius"` is in `ENV["JULIA_DEBUG"]`;
    setting `debug = true` without enabling debug logging prints a warning.
...
This algorithm supports 2 debugging message group names, which can be passed to
the environment variable `JULIA_DEBUG`:
...
spectral radius is not possible on GPUs.
"""
...
This algorithm is similar to a `StationaryIterativeSolve` with a
[`BlockArrowheadPreconditioner`](@ref), but it usually converges more quickly,
i.e., the spectral radius of its iteration matrix (`I - inv(P) * A`) tends to be
smaller. Roughly speaking, this is because it runs the
...
a `StationaryIterativeSolve` with a `BlockArrowheadPreconditioner` might be
faster.
