...
Convert a field defined on a `FiniteDifferenceSpace`,
`ExtrudedFiniteDifferenceSpace`, or a `MultiColumnFiniteDifferenceSpace` into a
vector of arrays, each of which corresponds to a column of the field. This is
done by calling [`column_field2array`](@ref) on each of the field's columns.
"""
