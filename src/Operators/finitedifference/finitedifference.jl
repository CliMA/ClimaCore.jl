...
"""
struct OneSided1stOrder <: AbstractBoundaryCondition end
...
"""
struct OneSided3rdOrder <: AbstractBoundaryCondition end
...
Supported boundary conditions are:
- [`OneSided1stOrder(x₀)`](@ref): uses the first-order downwind reconstruction to compute `x` on the left boundary, and the first-order upwind reconstruction to compute `x` on the right boundary.
