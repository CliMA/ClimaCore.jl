...
function stencil_interior(::GradientF2C, loc, idx, arg)
    Geometry.Covariant3Vector(1) ⊗ (getidx(arg, loc, idx + half) ⊟ getidx(arg, loc, idx - half))
end
...
function stencil_left_boundary(op::GradientF2C, ::Extrapolate, loc, idx, arg)
    space = axes(arg)
    @assert idx == left_center_boundary_idx(arg)
    Geometry.transform(Geometery.Covariant3Axis(), stencil_interior(op, loc, idx + 1, arg),  Geometry.LocalGeometry(space, idx))
end
function stencil_right_boundary(op::GradientF2C, ::Extrapolate, loc, idx, arg)
    space = axes(arg)
    @assert idx == right_center_boundary_idx(arg)
    Geometry.transform(Geometry.Covariant3Axis(), stencil_interior(op, loc, idx - 1, arg),  Geometry.LocalGeometry(space, idx))
end
...
function stencil_left_boundary(::GradientC2F, bc::SetGradient, loc, idx, arg)
    ...
    Geometry.transform(Geometry.Covariant3Axis(), bc.val, Geometry.LocalGeometry(space, idx))
end
