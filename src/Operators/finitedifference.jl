...
Base.@propagate_inbounds function getidx(
    ...
    v = vidx(space, idx)
    i, j, h = hidx
    return add_auto_broadcasters(field_data[CartesianIndex(i, j, 1, v, h)])
end

# Enable automatic nested broadcasting over boxed scalars
@inline getidx(parent_space, scalar::Tuple{T}, idx, hidx) where {T} =
    add_auto_broadcasters(scalar[1])
@inline getidx(parent_space, scalar::Ref, idx, hidx) =
    add_auto_broadcasters(scalar[])
@inline getidx(parent_space, field::Fields.PointField, idx, hidx) =
    add_auto_broadcasters(field[])
@inline getidx(parent_space, field::Fields.PointField, idx) =
    add_auto_broadcasters(field[])

# Enable automatic nested broadcasting over single-valued boundary conditions
@inline getidx(_, arg, _, _) = add_auto_broadcasters(arg)
