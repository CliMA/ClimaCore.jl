...
stencil_interior_width(::WeightedInterpolateF2C, weight, arg) =
    ((-half, half), (-half, half))
@inline function stencil_interior(::WeightedInterpolateF2C, loc, idx, hidx, weight, arg)
...
stencil_interior_width(::WeightedInterpolateC2F, weight, arg) =
    ((-half, half), (-half, half))
@inline function stencil_interior(::WeightedInterpolateC2F, loc, idx, hidx, weight, arg)
    w⁺ = getidx(weight, loc, idx + half, hidx)
    w⁻ = getidx(weight, loc, idx - half, hidx)
    a⁺ = getidx(arg, loc, idx + half, hidx)
    a⁻ = getidx(arg, loc, idx - half, hidx)
...
stencil_interior_width(::FluxCorrectionC2C, velocity, arg) =
    ((-half, +half), (-1, 1))
@inline function stencil_interior(::FluxCorrectionC2C, loc, idx, hidx, velocity, arg)
    space = axes(arg)
    θ⁺ = getidx(arg, loc, idx + 1, hidx)
    θ = getidx(arg, loc, idx, hidx)
    θ⁻ = getidx(arg, loc, idx - 1, hidx)
...
stencil_interior_width(::FluxCorrectionF2F, velocity, arg) =
    ((-half, +half), (-1, 1))
@inline function stencil_interior(::FluxCorrectionF2F, loc, idx, hidx, velocity, arg)
    space = axes(arg)
    θ⁺ = getidx(arg, loc, idx + 1, hidx)
    θ = getidx(arg, loc, idx, hidx)
    θ⁻ = getidx(arg, loc, idx - 1, hidx)
...
@inline fd3_curl(u₊::Geometry.Covariant1Vector, u₋::Geometry.Covariant1Vector, J) =
    Geometry.Contravariant2Vector((u₊.u₁ - u₋.u₁) / J)
@inline fd3_curl(u₊::Geometry.Covariant2Vector, u₋::Geometry.Covariant2Vector, J) =
...
@inline fd3_curl(u₊::Geometry.Covariant12Vector, u₋::Geometry.Covariant12Vector, J) =
...
@inline function stencil_left_boundary(::CurlC2F, bc::SetValue, loc, idx, hidx, arg)
    space = axes(arg)
    u₊ = getidx(arg, loc, idx + half, hidx)
...
@inline function stencil_right_boundary(::CurlC2F, bc::SetValue, loc, idx, hidx, arg)
    space = axes(arg)
    u = getidx(bc.val, loc, nothing, hidx)
    u₋ = getidx(arg, loc, idx - half, hidx)
    local_geometry = Geometry.LocalGeometry(space, idx, hidx)
...
@inline function stencil_left_boundary(::CurlC2F, bc::SetCurl, loc, idx, hidx, arg)
    return getidx(bc.val, loc, nothing, hidx)
end
@inline function stencil_right_boundary(::CurlC2F, bc::SetCurl, loc, idx, hidx, arg)
    return getidx(bc.val, loc, nothing, hidx)
end
...
Base.@propagate_inbounds function getidx(field::Fields.Field, loc::Location, idx, hidx)
...
function apply_stencil!(field_out, bc, hidx)
    space = axes(bc)
    if Topologies.isperiodic(Spaces.vertical_topology(space))
        @inbounds for idx in left_idx(space):(left_idx(space) + length(space) - 1)
