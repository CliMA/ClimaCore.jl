...
function columnwise_kernel!(
    ...
    if localmem_lg
        ...
        (ᶜspace_col, ᶠspace_col) =
            column_spaces(_ᶜY, _ᶠY, ᶠui, ᶜlg_arr, ᶠlg_arr)
    else
...
sync_threads(device::ClimaComms.AbstractCPUDevice) = nothing
...
"""
	column_lg_local_mem(space, ui, lg_arr)
...
"""
function column_lg_local_mem(space, ui, lg_arr)
    (i, j, _, _, h) = ui.I
...
function column_spaces(ᶜY, ᶠY, ui, ᶜlg_arr, ᶠlg_arr)
    (i, j, _, _, h) = ui.I
    colidx = Grids.ColumnIndex((i, j), h)
    ᶜlg_col = column_lg_local_mem(axes(ᶜY), ui, ᶜlg_arr)
    ᶠlg_col = column_lg_local_mem(axes(ᶠY), ui, ᶠlg_arr)
