...
elements (see [`ldg_laplacian_tendency!`](@ref)), and `weighted_dss!` is a
...
function scalar_laplacian!(::Grids.DG, out, χ, weight)
    ...
    # together (see `ldg_penalty_parameter`).
    τ = _laplacian_scratch_field(space, FT, :scalar_laplacian_penalty)
    ldg_penalty_parameter!(τ, κ; weight)
    return ldg_laplacian_tendency!(out, G_uv, R_uv, q, weight, κ, τ)
end
