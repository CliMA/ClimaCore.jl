...
# The error is a singleton exception type whose message is only generated by
# showerror on the host, since nested_broadcast can be called from GPU kernels,
# where neither string interpolation nor unique can be compiled.
struct UnequalNestedBroadcastLengthsError{lengths} <: Exception end
Base.showerror(
    io::IO,
    ::UnequalNestedBroadcastLengthsError{lengths},
) where {lengths} = print(
    io,
...
function _nested_broadcast(f::F, args) where {F}
    ...
    broadcastable_args = unrolled_flatmap(get_auto_broadcastable_tuple, unwrapped_args)
    lengths = unrolled_map(length, broadcastable_args)
    unrolled_allequal(lengths) || throw(UnequalNestedBroadcastLengthsError{lengths}())
