...
using ForwardDiff: ForwardDiff, DiffRules
...
"""
    allow_auto_broadcasting(bc)

Modifies a `Base.Broadcast.Broadcasted` wrapper by calling either
[`add_auto_broadcasters`](@ref) or [`drop_auto_broadcasters`](@ref) on its
arguments if doing so may prevent an error from being thrown.

When the [`unsafe_eltype`](@ref) of `bc` indicates that an error will be thrown,
an alternative `Broadcasted` wrapper is constructed with `add_auto_broadcasters`
applied to every argument, and then another `Broadcasted` wrapper is constructed
with `drop_auto_broadcasters` applied to every argument. If one of the new
...
function allow_auto_broadcasting(bc::Base.Broadcast.Broadcasted)
    ...
    bc′ = Base.Broadcast.Broadcasted(bc.style, wrapped_f, bc.args, bc.axes)
    unsafe_eltype(bc′) != Union{} && return bc′
    bc′′ = Base.Broadcast.Broadcasted(bc.style, unwrapped_f, bc.args, bc.axes)
    unsafe_eltype(bc′′) != Union{} && return bc′′
...
# Wrap each Type in a Base.Fix1 struct to ensure that it is constant propagated
