...
`ExtrudedFiniteDifferenceSpace` or a `MultiColumnFiniteDifferenceSpace`, the horizontal
grid and global geometry are kept and the hypsography is `Flat`; for an
`AbstractFiniteDifferenceSpace`, the result is a `FiniteDifferenceSpace`.
"""
