...
"""
    Perimeter3D <: AbstractPerimeter

Iterate over the perimeter degrees of freedom of a 3D spectral element.
"""
struct Perimeter3D{Nq} <: AbstractPerimeter end

"""
    Perimeter3D(Nq)

Construct a perimeter iterator for a 3D spectral element of degree `(Nq-1)`.
"""
Perimeter3D(Nq) = Perimeter3D{Nq}()
Adapt.adapt_structure(to, x::Perimeter3D) = x

function Base.iterate(perimeter::Perimeter3D{Nq}, loc = 1) where {Nq}
    if loc < 5
...
function Base.getindex(perimeter::Perimeter3D{Nq}, loc = 1) where {Nq}
...
nperimeter(::Perimeter3D{Nq}) where {Nq} = nperimeter2d(Nq)
Base.length(::Perimeter3D{Nq}) where {Nq} = nperimeter2d(Nq)
...
function SpectralElementSpace1D(
    ...
    for elem in 1:nelements
        ...
        for i in 1:Nq
            ξ = quad_points[i]
            # TODO: we need to massage the coordinate points because the grid is assumed 3D
...
Adapt.adapt_structure(to, space::SpectralElementSpace3D) =
    SpectralElementSpace3D(
...
"""
    SpectralElementSpace3D(topology, quadrature_style; enable_bubble)

Construct a `SpectralElementSpace3D` instance given a `topology` and `quadrature`. The
...
"""
function SpectralElementSpace3D(
    topology,
    quadrature_style;
    enable_bubble = false,
)
    ...
    if domain isa Domains.SphereDomain
        ...
        CoordType3D = Geometry.LatLongPoint{FT} # Domains.coordinate_type(topology)
        global_geometry =
    ...
    LG = Geometry.LocalGeometry{AIdx, CoordType3D, FT, SMatrix{2, 2, FT, 4}}
    ...
    for (lidx, elem) in enumerate(Topologies.localelems(topology))
        ...
        for i in 1:high_order_Nq, j in 1:high_order_Nq, k in 1:high_order_Nq
            ξ = SVector(high_order_quad_points[i], high_order_quad_points[j], high_order_quad_points[k])
            u, ∂u∂ξ =
        ...
        if enable_bubble
            if abs(elem_area - high_order_elem_area) ≤ eps(FT)
                for i in 1:Nq, j in 1:Nq, k in 1:Nq
                    ξ = SVector(quad_points[i], quad_points[j], quad_points[k])
                    ...
                    J = det(Geometry.components(∂u∂ξ))
                    WJ = J * quad_weights[i] * quad_weights[j] * quad_weights[k]
                    local_geometry_slab[i, j, k] =
                        Geometry.LocalGeometry(u, J, WJ, ∂u∂ξ)
                end
            else
                ...
                # Linear elements: Nq == 2 (SpectralElementSpace3D cannot have Nq < 2)
    ...
    if topology isa Topologies.Topology3D
    ...
    SG = Geometry.SurfaceGeometry{
        FT,
        Geometry.AxisVector{FT, Geometry.LocalAxis{AIdx}, SVector{3, FT}},
    }
...
nlevels(space::SpectralElementSpace3D) = Nq + 1
perimeter(space::SpectralElementSpace3D) =
    Perimeter3D(Quadratures.degrees_of_freedom(space.quadrature_style))

const RectilinearSpectralElementSpace3D = SpectralElementSpace3D{
    <:Topologies.Topology3D{
        <:ClimaComms.AbstractCommsContext,
        <:Meshes.RectilinearMesh,
    },
}

const CubedSphereSpectralElementSpace3D = SpectralElementSpace3D{
    <:Topologies.Topology3D{
        <:ClimaComms.AbstractCommsContext,
...
function compute_surface_geometry(
    ...
    # TODO Verify size over 3D space
    @assert size(local_geometry_slab) == (Nq, Nq, Nq, 1, 1, 1)
    i, j, k = Topologies.face_node_index(face, Nq, q, reversed)

    local_geometry = local_geometry_slab[i, j, k]
    ...
    elseif face == 3
        J * ∂ξ∂x[2, :] * quad_weights[i]
    elseif face == 5
        # TODO: This is wrong! Fix the placeholder statement
        J * ∂ξ∂x[2, :] * quad_weights[i]
    elseif face == 6
        # TODO: This is wrong! Fix the placeholder statement
        J * ∂ξ∂x[2, :] * quad_weights[i]
...
A view into a `SpectralElementSpace3D` for a single slab.
"""
...
const SpectralElementSpaceSlab3D =
    SpectralElementSpaceSlab{Q, DL} where {Q, DL <: DataLayouts.DataSlab3D}

nlevels(space::SpectralElementSpaceSlab1D) = 1
nlevels(space::SpectralElementSpaceSlab3D) = Nq+1
...
Base.@propagate_inbounds function column(space::SpectralElementSpace3D, i, j, h)
...
function all_nodes(space::SpectralElementSpace3D)
...
"""
    unique_nodes(space::SpectralElementField3D)
...
"""
unique_nodes(space::SpectralElementSpace3D) =
    unique_nodes(space, space.quadrature_style)

unique_nodes(space::SpectralElementSpace3D, quad::Quadratures.QuadratureStyle) =
    UniqueNodeIterator(space)
unique_nodes(space::SpectralElementSpace3D, ::Quadratures.GL) = all_nodes(space)

struct UniqueNodeIterator{S}
    space::S
end

Base.eltype(iter::UniqueNodeIterator{<:SpectralElementSpace3D}) =
    Tuple{Tuple{Int, Int}, Int}

function Base.length(iter::UniqueNodeIterator{<:SpectralElementSpace3D})
    space = iter.space
...
Base.iterate(::UniqueNodeIterator{<:SpectralElementSpace3D}) =
    ((1, 1), 1), ((1, 1), 1)
function Base.iterate(
    iter::UniqueNodeIterator{<:SpectralElementSpace3D},
    ((i, j), e),
)
    space = iter.space
    Nq = Quadratures.degrees_of_freedom(space.quadrature_style)
