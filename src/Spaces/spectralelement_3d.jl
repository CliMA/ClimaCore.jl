...
A three-dimensional space: within each element the space is represented as a polynomial.
"""
struct SpectralElementSpace3D{T, Q, G, D, IS, BS} <:
       AbstractSpectralElementSpace
    topology::T
    quadrature_style::Q
    local_geometry::G
...
"""
    SpectralElementSpace3D(topology, quadrature_style)

Construct a `SpectralElementSpace3D` instance given a `topology` and `quadrature`.
"""
function SpectralElementSpace3D(topology, quadrature_style)
    CoordType3D = Topologies.coordinate_type(topology)
    AIdx = Geometry.coordinate_axis(CoordType3D)
    FT = eltype(CoordType3D)
    nelements = Topologies.nlocalelems(topology)
    Nq = Quadratures.degrees_of_freedom(quadrature_style)
    ...
    LG = Geometry.LocalGeometry{CoordType3D, FT, Mxξ, Mξx}
    ...
    for elem in 1:nelements
        ...
        for i in 1:Nq, j in 1:Nq, k in 1:Nq
            ...
            ∂x∂ξ = ForwardDiff.jacobian(ξ) do ξ
                ...
                SVector(Geometry.component(x, 1), Geometry.component(x, 2), Geometry.component(x, 3))
            end
            J = det(∂x∂ξ)
            ∂ξ∂x = inv(∂x∂ξ)
...
nlevels(space::SpectralElementSpace3D) = 1
