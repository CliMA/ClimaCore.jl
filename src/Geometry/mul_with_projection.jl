...
@inline function recursively_find_dual_axes_for_projection(
    ::Type{X},
) where {X <: Union{Tuple, NamedTuple}}
    axes = map(recursively_find_dual_axes_for_projection, fieldtypes(X))
    return all(isnothing, axes) ? nothing : axes
end
