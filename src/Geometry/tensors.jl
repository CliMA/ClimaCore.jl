...
# `Components{Contravariant}`, a `CovariantVector` is one over
...
# Type-domain analogues of unrolled_allunique and unrolled_unique, which check
# each name against the preceding names instead of pushing new names into an
# accumulator like the implementations in UnrolledUtilities v0.1.9; see the
# "Kernel-reachable unrolled functions" unit test.
@inline is_repeated_name(names, n) =
    unrolled_any(m -> names[m] === names[n], StaticOneTo(n - 1))
@inline all_names_unique(names) =
    !unrolled_any(Base.Fix1(is_repeated_name, names), StaticOneTo(length(names)))
@inline unique_names(names) = unrolled_flatmap(
    n -> is_repeated_name(names, n) ? () : (names[n],),
    StaticOneTo(length(names)),
)

# The errors below are singleton exception types whose messages are only
# generated by showerror on the host, since basis computations can be compiled
# for GPUs, where string interpolation cannot be compiled.
struct DuplicateComponentNamesError{names} <: Exception end
Base.showerror(io::IO, ::DuplicateComponentNamesError{names}) where {names} =
    print(io, "Component names are not all unique: $names")

Components(::T, names) where {T} =
    all_names_unique(names) ? Components{T, names}() :
    throw(DuplicateComponentNamesError{names}())
...
dual(b::Components) =
    Components(dual_components_type(components_type(b)), component_names(b))
...
struct NoMetricError{T1, T2} <: Exception end
Base.showerror(io::IO, ::NoMetricError{T1, T2}) where {T1, T2} =
    print(io, "Metric is needed for change of basis: $T1 vs $T2")

struct ScalarBasisError{T} <: Exception end
Base.showerror(io::IO, ::ScalarBasisError{T}) where {T} =
    print(io, "Incompatible bases: one scalar vs $T vectors")

@noinline no_metric_error(::Type{T1}, ::Type{T2}) where {T1, T2} =
    throw(NoMetricError{T1, T2}())
@noinline scalar_error(::Type{T}) where {T} = throw(ScalarBasisError{T}())
...
# Use unrolled_flatmap instead of unrolled_filter because this is called from
# Tensor indexing, which can be compiled for GPUs; see the "Kernel-reachable
# unrolled functions" unit test.
axes_at_colons(bases, indices) =
    unrolled_flatmap((basis, index) -> index isa Colon ? (basis,) : (), bases, indices)
...
vector stored by its covariant components (`u₁`, `u₂`, `u₃`) along the dimensions `I`,
...
a vector stored by its contravariant components (`u¹`, `u²`, `u³`) along the dimensions
...
vector stored by its components (`u`, `v`, `w`) in the local orthonormal frame along the
dimensions `I`, with element type `T` and storage `S`. The concrete aliases `UVector`,
