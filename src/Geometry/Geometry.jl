...
reliably fold `_dual_axes_for_projection` — its recursion limiter widens the
recursive call to `Any` after a few levels of nesting (e.g. for the
`nested_type` entries of the `test_non_scalar_4` matrix-field broadcast, and
regardless of whether the components are traversed with `map` or
`unrolled_map`) — hence the generator, which evaluates it during compilation by
construction. The cost is a frozen method table: the generator cannot see
`_dual_axes_for_projection` methods defined after it, including any added by
downstream packages.
