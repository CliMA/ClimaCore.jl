_to_linear_index(A::AbstractArray, li, ci) =
    _to_linear_index(A, Base.to_indices(li, (ci,))...)
...
# Generic cases
_sub2ind(dims::Base.DimsInteger, I::Integer...) =
    (@inline; _sub2ind_recurse(dims, 1, 1, I...))
_sub2ind(inds::Base.Indices, I::Integer...) =
    (@inline; _sub2ind_recurse(inds, 1, 1, I...))
# In 1d, there's a question of whether we're doing cartesian indexing
# or linear indexing. Support only the former.
_sub2ind(inds::Base.Indices{1}, I::Integer...) = throw(
    ArgumentError("Linear indexing is not defined for one-dimensional arrays"),
)
_sub2ind(inds::Tuple{Base.OneTo}, I::Integer...) =
    (@inline; _sub2ind_recurse(inds, 1, 1, I...)) # only OneTo is safe
