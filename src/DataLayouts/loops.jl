...
@inline foreach_slice(
    ...
    foreach_slice(DataScope(args...), op, f, args...; mask, enumerate) :
...
@inline unfused_slice_loop(scope, op::O, f::F, args...; mask, enumerate) where {O, F} =
...
"""
    foreach_point(f, args...; [mask], [enumerate])
...
for op in (:level, :slab, :column)
    @eval begin
        """
            foreach_$($op)(f, args...; [mask], [enumerate])
