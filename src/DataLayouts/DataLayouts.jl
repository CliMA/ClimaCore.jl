...
import StaticArrays: SOneTo, MArray, SArray, Size
import ClimaComms
import MultiBroadcastFusion as MBF
import Adapt
import UnrolledUtilities: generic_getindex
...
@inline ncomponents(data::AbstractData{S}) where {S} =
    storage_length(eltype(parent(data)), S)
...
function replace_storage(data::AbstractData, ::Type{S}, ::Type{B}) where {S, B}
    D = field_dim(singleton(data))
    params = Base.tail(type_params(data))
    new_array_size = Base.setindex(size(parent(data)), storage_length(B, S), D)
    new_array = similar(parent(data), B, new_array_size...)
    return union_all(singleton(data)){S, params...}(new_array)
end

replace_basetype(data::AbstractData{S}, ::Type{B}) where {S, B} =
    replace_storage(data, replace_type_parameter(S, eltype(parent(data)), B), B)
...
function IJKFVH{S, Nij, Nk, Nv}(
    ...
    @assert size(array, 4) == storage_length(T, S)
...
function IJKFVH{S}(
    ...
    Nf = storage_length(eltype(ArrayType), S)
...
function IJFH{S, Nij}(array::AbstractArray{T, 4}) where {S, Nij, T}
    ...
    @assert size(array, 1) == Nij
    @assert size(array, 2) == Nij
    @assert size(array, 3) == storage_length(T, S)
...
function IJFH{S, Nij}(::Type{ArrayType}, Nh::Integer) where {S, Nij, ArrayType}
    T = eltype(ArrayType)
    IJFH{S, Nij}(ArrayType(undef, Nij, Nij, storage_length(T, S), Nh))
end
...
function IJHF{S, Nij}(::Type{ArrayType}, Nh::Integer) where {S, Nij, ArrayType}
    T = eltype(ArrayType)
    IJHF{S, Nij}(ArrayType(undef, Nij, Nij, Nh, storage_length(T, S)))
end
...
function IFH{S, Ni}(array::AbstractArray{T, 3}) where {S, Ni, T}
    ...
    @assert size(array, 2) == storage_length(T, S)
...
function IFH{S, Ni}(::Type{ArrayType}, Nh::Integer) where {S, Ni, ArrayType}
    T = eltype(ArrayType)
    IFH{S, Ni}(ArrayType(undef, Ni, storage_length(T, S), Nh))
end
...
function IHF{S, Ni}(::Type{ArrayType}, Nh::Integer) where {S, Ni, ArrayType}
    T = eltype(ArrayType)
    IHF{S, Ni}(ArrayType(undef, Ni, Nh, storage_length(T, S)))
end
...
function DataF{S}(array::AbstractVector{T}) where {S, T}
    check_basetype(T, S)
    @assert size(array, 1) == storage_length(T, S)
...
function VF{S, Nv}(::Type{ArrayType}, nelements) where {S, Nv, ArrayType}
    T = eltype(ArrayType)
    check_basetype(T, S)
    VF{S, Nv}(ArrayType(undef, nelements, storage_length(T, S)))
end
...
function VIJHF{S, Nv, Nij}(array::AbstractArray{T, 5}) where {S, Nv, Nij, T}
    ...
    @assert size(array, 1) == Nv
    @assert size(array, 2) == size(array, 3) == Nij
    @assert size(array, 5) == storage_length(T, S)
