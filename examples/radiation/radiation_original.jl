...
function do_stuff(param_set)
    ...
    integrals = map(1:ncol) do icol
        ...
        map(face_pressure[:, icol]) do p
            FT(integrate(spline, p, face_pressure[1, icol])) ./
                FT(Planet.grav(param_set))
        end
    end
    face_elevations =
        DA(sum(integrals) ./ (length(integrals) * FT(Planet.grav(param_set))))
    ...
    heating_rate_factor = DA(specific_volume_over_temperature ./ specific_heat_capacity)
...
Δt = 60. * 60.
prob = ODEProblem(tendency!, T, (0., 24. * Δt), p)
sol = solve(
    prob,
    Euler(), # SSPRK33(),
    dt = Δt,
