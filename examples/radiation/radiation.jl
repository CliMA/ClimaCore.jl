...
function get_var(string)
    var = ds_input[string]
    if ndims(var) == 3
        array = reshape(var[:], size(var, 1), ncol)
    elseif ndims(var) == 2
        if "site" in dimnames(var) && "expt" in dimnames(var)
            return reshape(var[:], ncol)
        elseif "site" in dimnames(var)
            array = repeat(var[:]; outer = (1, nexpt))
        else # "expt" in dimnames(var)
            array = repeat(var[:]; inner = (1, nsite))
        end
    else # ndims(var) == 1
    ...
    return reverse(array, dims = 1)
end

init_dict = Dict(keys(ds_input) .=> get_var.(keys(ds_input)))
pressure = init_dict["pres_layer"]
face_pressure = init_dict["pres_level"]
temperature = init_dict["temp_layer"]
volume_mixing_ratio_h2o = init_dict["water_vapor"]
...
vmrs = map((
    ...
    units = parse(FT, ds_input[input_gas_name].attrib["units"])
    (
        Symbol("volume_mixing_ratio_" * lookup_gas_name),
        (z, icol) -> init_dict[input_gas_name][icol] .* units,
    )
end
...
rrtmgp_model = RRTMGPModel(
    ...
    surface_emissivity = (ibnd_lw, icol) -> init_dict["surface_emissivity"][icol],
    solar_zenith_angle = NaN,
    top_of_atmosphere_dir_dn_sw_flux = NaN,
    dir_sw_surface_albedo = (ibnd_sw, icol) -> init_dict["surface_albedo"][icol],
    dif_sw_surface_albedo = (ibnd_sw, icol) -> init_dict["surface_albedo"][icol],
    pressure = pressure,
    temperature = temperature,
    surface_temperature = init_dict["surface_temperature"],
    latitude = init_dict["lat"],
    volume_mixing_ratio_h2o = volume_mixing_ratio_h2o,
    volume_mixing_ratio_o3 = init_dict["ozone"],
    vmrs...,
