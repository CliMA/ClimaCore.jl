...
function ∑tendencies!(dT, T, z, t)
    ...
    bc_gb = Operators.SetGradient(FT(∇gaussian(z₀,t; ν = ν, δ=δ, 𝓌 = 𝓌, μ = μ)))
    bc_gt = Operators.SetGradient(FT(∇gaussian(z₁,t; ν = ν, δ=δ, 𝓌 = 𝓌, μ = μ)))
...
#   return @. dT = -∂(UB(V, ic2f(T)))
    ...
    gradc2f = Operators.GradientC2F(bottom = bc_vb,
                                    top = bc_gt)
...
anim = Plots.@animate for (nt,u) in enumerate(sol.u)
    Plots.plot(u, xlim = (0, 1), ylim = (-1,10), title = "$(nt-1) s", lc=:black, 
               lw=2, 
               ls=:dash, 
               label = "Approx Sol.", 
               legend=:outerright, 
               ...
               xlabel="T(z)", 
               ylabel="z")
    Plots.plot!(gaussian.(zp,nt-1;μ = μ, δ=δ, ν=ν, 𝓌=𝓌),zp, xlim = (0, 1), ylim = (-1,10), title = "$(nt) s", lc=:red, lw=2, label="Analytical Sol.", m=:x)
end
Plots.mp4(anim, joinpath(path, "adv_diff.mp4"), fps = 10)
Plots.png(Plots.plot(sol.u[end], xlim = (0, 1)), joinpath(path, "adv_diff_end.png"))
