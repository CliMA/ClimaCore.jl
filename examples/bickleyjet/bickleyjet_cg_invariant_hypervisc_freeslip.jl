...
function rhs!(dydt, y, _, t)
    ...
    Operators.add_numerical_flux_boundary!(
        ...
        contravariant_normal = Geometry.Contravariant12Vector(normal, local_geometry)
        -dot(y.u, contravariant_normal) * contravariant_normal
    end
    # CovariantVector(normal, local_geometry)
    Spaces.weighted_dss!(dydt)
    return dydt
end
#=
Turn cg bickley jet w hyperviscosity & weak divergence into a channel flow (freeslip/noslip)

Adding numerical boundary flux to mirror the velocity at the top or bottom wall.
    Why are we not directly modifying the state in the case of Dirichlet BC?
    Are we modifying dydt correctly? (flux = ρ*u?)
    Why is dot() erroring?
    Are we using the correct normal and obtaining it in the most direct way?
=#
