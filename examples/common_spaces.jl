...
Base.@kwdef struct CubedSphere{FT} <: Space{FT}
    radius::FT
    pelem::Int # number of elements along side of each panel (6 panels in total)
    npoly::Int
end
...
function make_space((; radius, pelem, npoly)::CubedSphere)
    domain = Domains.SphereDomain(radius)
    mesh = Meshes.EquiangularCubedSphere(domain, pelem)
    topology = Topologies.Topology2D(mesh)
    quad = Spaces.Quadratures.GLL{npoly + 1}()
    return Spaces.SpectralElementSpace2D(topology, quad)
end
