...
remaining_cache(Y, dt) = merge(
    baroclinic_wave_cache(Y, dt),
    held_suarez_cache(Y, dt),
    final_adjustments_cache(Y, dt; sponge = false),
)
...
function remaining_tendency!(dY, Y, p, t)
    ...
    final_adjustments!(dY, Y, p, t; use_flux_correction = false, sponge = false)
    return dY
end
