...
function vertical_warp!(space::Spaces.FaceExtrudedFiniteDifferenceSpace, Z::Fields.FaceExtrudedFiniteDifferenceField)
    @assert space === axes(Z)
    ...
    for h = 1:Nh, j = 1:Nj, i = 1:Ni
        ...
        for v = 1:Nv+1
            ...
            Δz = v == 1 ? 2*(cZ_column[v] - Z_column[v]) :
            ...
            face_column[v] = Geometry.LocalGeometry(
                coord,
                J,
                W*J,
                ∂x∂ξ
        ...
        for v = 1:Nv
            ...
            W = local_geom.WJ / local_geom.J
            J = det(Geometry.components(∂x∂ξ))
            center_column[v] = Geometry.LocalGeometry(
                coord,
                J,
                W*J,
                ∂x∂ξ
...
function reconstruct_metric(∂x∂ξ::Geometry.Axis2Tensor{T, Tuple{Geometry.UWAxis, Geometry.Covariant13Axis}}, ∇z::Geometry.Covariant1Vector, Δz::Real) where {T}
    Geometry.AxisTensor(
        axes(∂x∂ξ),
    @SMatrix [
...
function reconstruct_metric(∂x∂ξ::Geometry.Axis2Tensor{T, Tuple{Geometry.UVWAxis, Geometry.Covariant123Axis}}, ∇z::Geometry.Covariant12Vector, Δz::Real) where {T}
    Geometry.AxisTensor(
        axes(∂x∂ξ),
    @SMatrix [
