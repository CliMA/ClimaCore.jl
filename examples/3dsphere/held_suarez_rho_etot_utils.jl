...
const κ₄ = 2.0e17 # m^4/s hyperdiffusion
...
function rhs!(dY, Y, p, t)
    # p.Ycopy .= Y # uncomment if want to debug the field before crashing
    @unpack P, Φ, ∇Φ, c_coords, zmax, z_D, dt = p
    ...
    # 4) held_suarez forcing
    φ = @. deg2rad(c_coords.lat)
    ...
    T_equil =
        @. (T_equator - ΔT_y * sin(φ)^2 - Δθ_z * log(σ) * cos(φ)^2) * exner_p
    ...
    @. dρ += fcc(fw, cρ)
    @. dρe_tot += fcc(fw, cρe_tot)

    # 6) sponge for uₕ
    uₕ_ref =
        Geometry.Covariant12Vector.(
            map(
                coord -> Geometry.UVVector(u(coord.lat, coord.z), 0.0),
                c_coords,
            ),
        )

    αₘ = @. ifelse(c_coords.z > z_D, 1 / 20 / dt, FT(0.0))
    β = @. αₘ * sin(0.5 * π * (c_coords.z - z_D) / (zmax - z_D))^2
