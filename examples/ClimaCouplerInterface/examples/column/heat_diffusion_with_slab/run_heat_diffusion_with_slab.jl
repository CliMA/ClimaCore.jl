...
function coupler_solve!(stepping, ics, parameters)
    ...
    for t in (t_start : Δt_cpl : t_end)
        ...
        atm_F_sfc .= [0.0] # surfce flux to be accumulated #NB: this doesn't reset the flux 
...
# T_atm_end = sol_atm.u[end].x[1] 
# T_lnd_end = sol_lnd.u[end][1] .+ sol_atm.u[end].x[1] .* 0.0 
# Plots.png(Plots.plot( [T_lnd_end T_atm_end], labels = ["L" "A"]), joinpath(path, "T_both_end.png"))
...
Plots.png(Plots.plot(sol_lnd.t, [v1 v2], labels = ["L" "A"]), joinpath(path, "heat_both_surface_time.png"))
