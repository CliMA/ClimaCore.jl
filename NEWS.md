...
ClimaCore's first stable release. Names that are neither exported nor documented remain internal and may change in
any release.

Apart from the removals listed below, v1.0.0 is API-identical to v0.16.2;
...
- ![][badge-💥breaking] Removed the backwards-compatibility shims and
  ...
  but was never deprecated, is also removed. Its only callers were the
  `Remapping` interpolation kernels, which now read the field directly with
  `field[v, i, j, h]` through the internal helper `Remapping.node_value`.
...
- ![][badge-🐛bugfix] Fixed a GPU compilation failure (`gpu_gc_pool_alloc` and
  ...
  nothing-or-integer accumulator. All other kernel-reachable calls to the accumulator-based
  `unrolled_filter`, `unrolled_unique`, and `unrolled_allunique` (in `Geometry`
  basis computations and in `Utilities.nested_broadcast`) were converted to
  per-item checks or `unrolled_flatmap`, and a unit test now forbids these
  functions in kernel-reachable directories. Errors thrown from kernel-reachable
  code in `DataLayouts`, `Geometry`, and `Utilities` are now singleton exception
  types (`InvalidSubscopeError`, `NonOverlappingScopesError`, `NoMetricError`,
  `ScalarBasisError`, `DuplicateComponentNamesError`,
  `UnequalNestedBroadcastLengthsError`) whose messages include the relevant
  types but are only generated by `showerror` on the host. All kernel-reachable
  `unrolled_reduce` calls pass their init values positionally, since kwcalls do
  not always specialize during GPU compilation of wide broadcast expressions.
  A unit test materializes wide fused broadcasts that mix 0-dimensional and
  3-dimensional layouts of heterogeneous element types, and a 1-GPU Buildkite
