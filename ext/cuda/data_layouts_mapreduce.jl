...
function mapreduce_cuda(
    ...
    # each and we perform a block-wise reduction, with each block writing to an
    # array of (block-)shared memory. This array has the same size as the block,
    # ie, it is as long as many threads are available. Processing multiple
    # points means that we apply the reduction to the point with index
    # reduction[thread_index] = f(thread_index, thread_index + OFFSET), with
    # various OFFSETS that depend on `n_ops_on_load` and block size.
    #
