import ClimaCore.Operators:
    columnwise!, sync_threads, columnwise_kernel!, universal_index_columnwise

sync_threads(device::ClimaComms.CUDADevice) = CUDA.sync_threads()

local_mem(
    device::ClimaComms.CUDADevice,
    ::Type{T},
    ::Val{dims},
) where {T, dims} = CUDA.CuStaticSharedArray(T, Operators.__size(dims))
...
function columnwise!(
    ...
    threads = (ᶠNv,)
    blocks = (Nh, Ni * Nj)
    kernel(ᶜYₜ, ᶠYₜ, ᶜY, ᶠY, p, t; threads, blocks)
end
