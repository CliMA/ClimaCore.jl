...
max_eager_shmem_per_thread(
    bc::StencilBroadcasted{S, <:Operators.NonLinearAdvectionOperator},
) where {S} = max(
...
The type that the `NonLinearAdvectionOperator` method of `calc_level_val` writes into
...
"""
    calc_level_val(bc::StencilBroadcasted{<:Any, <:NonLinearAdvectionOperator}, hidx, space)
...
Base.@propagate_inbounds function calc_level_val(
    ...
    S,
    Op <: Operators.NonLinearAdvectionOperator,
    BC <: StencilBroadcasted{S, Op},
}
    op = bc.op
    v = threadIdx().x
    ...
    CUDA.sync_threads()
    stencil_vals =
        @inbounds advection_gather(width, shmem, v³, v, n_faces, periodic, col_offset)
...
`stencil_interior(::NonLinearAdvectionOperator, ...)`: out-of-range center indices are
