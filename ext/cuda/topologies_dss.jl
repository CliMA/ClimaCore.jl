...
function dss_load_perimeter_data_kernel!(
    ...
    sizep = (nlevels, nperimeter, nfidx, nelems) # specialize on VIJFH data
    CI = CartesianIndex
...
function Topologies.load_from_recv_buffer!(
    ...
    if nrecv > 0
        ...
        threads = threads_via_occupancy(load_from_recv_buffer_kernel!, args)
        p = linear_partition(nitems, threads)
        auto_launch!(
...
function load_from_recv_buffer_kernel!(
    ...
    if gidx ≤ nlevels * nfid * nrecv
        ...
        ci_data = CartesianIndex(DataLayouts.to_data_specific_field(s, ci.I))
        CUDA.@atomic pperimeter_data[ci_data] +=
            getindex_field(perimeter_data, ci)
    end
    return nothing
end
...
function Topologies.dss_ghost!(
    ...
    if nghostvertices > 0
        ...
        threads = threads_via_occupancy(dss_ghost_kernel!, args)
        p = linear_partition(nitems, threads)
        auto_launch!(
            dss_ghost_kernel!,
            args;
