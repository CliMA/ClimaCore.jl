...
The output follows the Exodus II specification
([EXODUS II: A finite element data model](https://www.osti.gov/biblio/10102115-exodus-ii-finite-element-data-model))
as far as TempestRemap requires it. The generated meshes use a different ordering of
nodes and elements than meshes generated by TempestRemap itself.

With a distributed topology under MPI, call this function on a single process only.

# Keyword Arguments

  - `normalize_coordinates = true`: if `true`, normalize the vertex coordinates onto the
    unit sphere, as TempestRemap requires.
"""
