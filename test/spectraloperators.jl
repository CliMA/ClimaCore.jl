...
domain = Domains.RectangleDomain(-pi..pi, -pi..pi; x1periodic=true, x2periodic=true)
...
@testset "curl" begin
    ...
    curlv_ref = .- 3 .* sin.(3 .* coords.x1 .+ 4 .* coords.x2) .- 2 .* cos.(coords.x1 .+ 2 .* coords.x2)
...
@testset "div" begin
    ...
    divv_ref = cos.(coords.x1 .+ 2 .* coords.x2) .- 2 .* sin.(3 .* coords.x1 .+ 2 .* coords.x2)
