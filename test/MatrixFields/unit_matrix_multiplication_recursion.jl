...
@testset "matrix multiplication recursion" begin
    ...
    # every entry.
    @test !any(isnan, parent(dest_field))
    # With K ≡ 0 the product term vanishes, so the only nonzero entry is the
    # divergence of the prescribed top-boundary flux in the top cell.
    @test count(!iszero, parent(dest_field)) == 1
end
