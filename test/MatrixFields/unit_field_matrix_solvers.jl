...
# Measure allocations from within dedicated functions. Measuring inline in
# `test_field_matrix_solver` makes the result depend on how much of the
# optimizer's inlining/escape-analysis budget the rest of that (large) function
# has used up, which reports spurious allocations of a few tens of bytes.
@noinline mul_allocs(b, A, x) = @allocated mul!(b, A, x)
@noinline ldiv_allocs(x, A, b) = @allocated ldiv!(x, A, b)
...
function test_field_matrix_solver(; test_name, alg, A, b, use_rel_error = false)
    @testset "$test_name" begin
        ...
        if get(ENV, "CLIMACORE_BENCHMARK_SOLVERS", "false") == "true"
            solve_time =
            ...
            if alg isa MatrixFields.StationaryIterativeSolve && alg.correlated_solves
                for _ in 1:30
                    ClimaComms.@cuda_sync comms_device ldiv!(x, A′, b)
                end
            else
                ClimaComms.@cuda_sync comms_device ldiv!(x, A′, b)
            end
            b_test = similar(b)
            @test zero(b) isa typeof(b)
            ClimaComms.@cuda_sync comms_device mul!(b_test, A′, x)
            timing_info = ""
        end
        ...
        USING_CUDA || @test mul_allocs(b_test, A′, x) == 0
    end
end
