...
@testset "backwards compatibility with `value_type` attribute" begin
    ...
    InputOutput.HDF5Writer(filename, context) do writer
        ...
        delete_attribute(obj, "field_eltype") # remove the field_eltype attribute
        write_attribute(obj, "value_type", string(eltype(Y))) # add the deprecated value_type attribute
    end
