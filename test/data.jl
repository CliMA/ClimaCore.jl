...
@testset "IFH" begin
    ...
    data_slab = slab(data, 1)
    @test data_slab[1] ==
          (Complex(array[1, 1, 1], array[1, 2, 1]), array[1, 3, 1])
    
    data_slab[1] = (Complex(-1.0, -2.0), -3.0)
    @test array[1, 1, 1] == -1.0
    @test array[1, 2, 1] == -2.0
    @test array[1, 3, 1] == -3.0

    subdata_slab = data_slab.:2
    @test subdata_slab[1] == -3.0
    subdata_slab[1] = -5.0
    @test array[1, 3, 1] == -5.0

    @test sum(data.:1) ≈ Complex(sum(array[:, 1, :]), sum(array[:, 2, :])) atol =
        10eps()
end

@testset "broadcasting 1D between data object + scalars" begin
    FT = Float64
    data1 = ones(FT, 2, 2, 2)
    S = Complex{Float64}
    data1 = IFH{S, 2}(data1)
    ...
    @test res isa IFH{S}
    @test parent(res) ==
          FT[f == 1 ? 2 : 1 for i in 1:2, f in 1:2, h in 1:2]

    @test sum(res) == Complex(8.0, 4.0)
    @test sum(Base.Broadcast.broadcasted(+, data1, 1)) == Complex(8.0, 4.0)
end

@testset "broadcasting 2D between data object + scalars" begin
    FT = Float64
...
@testset "broadcasting 2D assignment from scalar" begin
    FT = Float64
    S = Complex{FT}
...
@testset "broadcasting 2D between data objects" begin
    FT = Float64
    data1 = ones(FT, 2, 2, 2, 2)
...
@testset "broadcasting 2D complicated function" begin
    FT = Float64
