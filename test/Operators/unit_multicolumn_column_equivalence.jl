...
# Run every operation on `ᶜspace` and return the resulting fields by name
function results(ᶜspace)
    ...
    # Horizontal (spectral) operators, which evaluate to zero on a column
...
@testset "Multi-column space matches single columns" begin
    ...
    kwargs = (;
        z_elem = 8, z_min = zero(FT), z_max = FT(4), radius = FT(100),
        z_mesh, staggering = Grids.CellCenter(), device,
    )
    ᶜmulti = MultiColumnSpace(; points, kwargs...)
    ᶜsingle = Spaces.CenterFiniteDifferenceSpace(device, z_mesh)

    # Apply operators and compare single and multi column results
    multi = results(ᶜmulti)
    single = results(ᶜsingle)
    ncols = length(points)
    for name in keys(multi), h in 1:ncols
        name == :ᶜsum && continue
        @test column_values(multi[name], h) ≈ values_of(single[name])
    end
    @test multi.ᶜsum ≈ ncols * single.ᶜsum
end
