...
# Boundary flux must be isbits (no captured `Type{FT}`) for the CUDA kernel.
bflux(normal, (q⁻,)) = q⁻ * (Geometry.UVVector(zero(q⁻), one(q⁻))' * normal)
...
@testset "DG face kernels: CPU vs GPU equivalence" begin
    ...
    TU.@test_precisions FT begin
        ...
        @testset "cartesian tensor divergence [pure-2D sphere, $FT]" begin
            ...
            d_cpu = Operators.cartesian_tensor_divergence(T_cpu, completion)
            d_dev = Operators.cartesian_tensor_divergence(T_dev, completion)
