...
@testset "FD shared memory: dispatch" begin # this ensures that we exercise the correct code-path
    ...
    div = Operators.DivergenceF2C(;
        top = Operators.SetValue(10),
        bottom = Operators.SetValue(10),
    )
    bc = @. lazy(div(ᶠinterp(Geometry.WVector(c))))
    @test Operators.any_fd_shmem_supported(bc)
end
...
@testset "Correctness column" begin
    ...
    @test compare_cpu_gpu(fields_cpu.ᶠout1, fields.ᶠout1);                @test !is_trivial(fields_cpu.ᶠout1)
    @test compare_cpu_gpu(fields_cpu.ᶠout2, fields.ᶠout2);                @test !is_trivial(fields_cpu.ᶠout2)
    ...
    @test compare_cpu_gpu(fields_cpu.ᶜout13, fields.ᶜout13);              @test !is_trivial(fields_cpu.ᶜout13)
    @test compare_cpu_gpu(fields_cpu.ᶜout14, fields.ᶜout14);              @test !is_trivial(fields_cpu.ᶜout14)
    ...
    @test compare_cpu_gpu(fields_cpu.ᶠout4_cov, fields.ᶠout4_cov);        @test !is_trivial(fields_cpu.ᶠout4_cov)
    @test compare_cpu_gpu(fields_cpu.ᶠout5_cov, fields.ᶠout5_cov);        @test !is_trivial(fields_cpu.ᶠout5_cov)
end
