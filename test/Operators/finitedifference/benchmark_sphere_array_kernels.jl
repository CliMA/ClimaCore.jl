...
# function column_op_2mul_1add!(x, y, D, U, ::Val{Nv}, ::Val{Nij}, ::Val{Nh}) where {Nv, Nij, Nh}
#     y1 = @view y[1:(end - 1)]
#     y2 = @view y[2:end]
...
function sphere_op_2mul_1add_cuda!(x, y, D, U)
    ...
    Nh = size(x, 5)
    N = length(x)
    kernel = CUDA.@cuda always_inline = true launch = false sphere_op_2mul_1add_cuda_kernel!(
        x, y, D, U,
        Val(Nv),
        Val(Nij),
        Val(Nh),
...
function sphere_op_2mul_1add_cuda_kernel!(x, y, D, U, ::Val{Nv}, ::Val{Nij}, ::Val{Nh}, ::Val{N}) where {Nv, Nij, Nh, N}
    @inbounds begin
        tidx = thread_index()
