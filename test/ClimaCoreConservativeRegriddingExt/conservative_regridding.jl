...
@testset "test integrate_each_element" begin
    ...
    @test isapprox(sum(integral_each_element), sum(ones_field), atol = 1e-14)
    ...
    @test isapprox(sum(integral_each_element), sum(field), atol = 1e-11)
end

@testset "test get_value_per_element!" begin
    field = deepcopy(Fields.coordinate_field(src_space).lat)
    ones_field = Fields.ones(src_space)
...
@testset "test set_value_per_element!" begin
    ...
    # Compare with mean instead of sum because value_per_element is an array (not Field)
    @test isapprox(mean(field), mean(value_per_element), atol = 1e-14)
    @test all(parent(field) .== 1.0)
end
...
@testset "test regrid!" begin
    src_field = deepcopy(Fields.coordinate_field(src_space).lat)
    ...
    @test isapprox(sum(dst_field), sum(src_field), atol = 1e-11)
...
@testset "test regrid! of a constant field" begin
    src_field = deepcopy(ones(src_space))
    dst_field = Fields.zeros(src_space)
...
@testset "test regrid! from source to destination and back" begin
    ...
    @test isapprox(sum(src_field), sum(dst_field), atol = 1e-11)
end
