...
@testset "2D terrain-following space (LinearAdaption)" begin
    FT = Float64
    ...
    z_surface =
        Geometry.ZPoint.(sin.(Fields.coordinate_field(horzspace).x) .+ 1)
...
@testset "physical_z <-> ref_z round trip" begin
    FT = Float64
    ...
    @test Hypsography.physical_z_to_ref_z(
        Grids.Flat(),
        Hypsography.ref_z_to_physical_z(Grids.Flat(), z_ref1, z_top1),
        z_top1,
    ).z ≈ z_ref1.z

    # Linear adaption
    linear_adaption = Hypsography.LinearAdaption(z_surface1)
    @test Hypsography.physical_z_to_ref_z(
        linear_adaption,
        Hypsography.ref_z_to_physical_z(linear_adaption, z_ref1, z_top1),
        z_top1,
    ).z ≈ z_ref1.z
end
