...
@testset "copyto! with Nf = 1" begin
    ...
    for Nv in (1, Nv), (Ni, Nj) in ((1, 1), (1, Nij), (Nij, Nij)), Nh in (1, Nh)
        for D in (DataLayouts.VIJFH, DataLayouts.VIJHF)
            data = D{FT, Nv, Ni, Nj, Nh == 1 ? 1 : missing}(A, Nh)
            benchmarkcopyto!(bm, device, data, 3)
            @test all(parent(data) .== 3)
        end
    end
    for Nv in (1, Nv), Ni in (1, Nij), Nh in (1, Nh)
        data = DataLayouts.VIH1{FT, Nv, Ni, Nh == 1 ? 1 : missing}(A, Nh)
        benchmarkcopyto!(bm, device, data, 3)
        @test all(parent(data) .== 3)
    end
    for (Ni, Nj) in ((1, 1), (1, Nij), (Nij, Nij)), Nh in (1, Nh)
        data = DataLayouts.IH1JH2{FT, Ni, Nj, Nh == 1 ? 1 : missing}(A, Nh)
        benchmarkcopyto!(bm, device, data, 3)
        @test all(parent(data) .== 3)
    end
