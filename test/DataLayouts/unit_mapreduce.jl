...
@testset "mapreduce with Nf = 1" begin
    ...
    test_mapreduce_1!(context, data)
    # data = IFH{S}(ArrayType{FT}; Ni,Nh);               test_mapreduce_1!(context, data)
    # data = IJF{S}(ArrayType{FT}; Nij);                 test_mapreduce_1!(context, data)
    # data = IF{S}(ArrayType{FT}; Ni);                   test_mapreduce_1!(context, data)
    data = VF{S}(ArrayType{FT}; Nv)
    test_mapreduce_1!(context, data)
    data = VIJFH{S}(ArrayType{FT}; Nv, Nij, Nh)
    test_mapreduce_1!(context, data)
    # data = VIFH{S}(ArrayType{FT}; Nv,Nij,Nh);                  test_mapreduce_1!(context, data)
    # data = DataLayouts.IJKFVH{S}(ArrayType{FT}; Nij,Nk,Nv,Nh); test_mapreduce_1!(context, data) # TODO: test
    # data = DataLayouts.IH1JH2{S}(ArrayType{FT}; Nij);             test_mapreduce_1!(context, data) # TODO: test
end
...
@testset "mapreduce with Nf > 1" begin
    ...
    data = IJFH{S}(ArrayType{FT}; Nij, Nh)
    test_mapreduce_2!(context, data)
    # data = IFH{S}(ArrayType{FT}; Ni,Nh);               test_mapreduce_2!(context, data)
    # data = IJF{S}(ArrayType{FT}; Nij);                 test_mapreduce_2!(context, data)
    # data = IF{S}(ArrayType{FT}; Ni);                   test_mapreduce_2!(context, data)
    data = VF{S}(ArrayType{FT}; Nv)
    test_mapreduce_2!(context, data)
    data = VIJFH{S}(ArrayType{FT}; Nv, Nij, Nh)
    test_mapreduce_2!(context, data)
    # data = VIFH{S}(ArrayType{FT}; Nv,Nij,Nh);                  test_mapreduce_2!(context, data)
    # TODO: test this
    # data = DataLayouts.IJKFVH{S}(ArrayType{FT}; Nij,Nk,Nv,Nh); test_mapreduce_2!(context, data) # TODO: test
    # data = DataLayouts.IH1JH2{S}(ArrayType{FT}; Nij);             test_mapreduce_2!(context, data) # TODO: test
end
...
@testset "mapreduce views with Nf > 1" begin
    ...
    data = IJFH{S}(ArrayType{FT}; Nij, Nh)
    test_mapreduce_2!(context, data_view(data))
    # data = IFH{S}(ArrayType{FT}; Ni,Nh);               test_mapreduce_2!(context, data_view(data))
    # data = IJF{S}(ArrayType{FT}; Nij);                 test_mapreduce_2!(context, data_view(data))
    # data = IF{S}(ArrayType{FT}; Ni);                   test_mapreduce_2!(context, data_view(data))
    data = VF{S}(ArrayType{FT}; Nv)
    test_mapreduce_2!(context, data_view(data))
    data = VIJFH{S}(ArrayType{FT}; Nv, Nij, Nh)
    test_mapreduce_2!(context, data_view(data))
    # data = VIFH{S}(ArrayType{FT}; Nv,Nij,Nh);                  test_mapreduce_2!(context, data_view(data))
    # TODO: test this
    # data = DataLayouts.IJKFVH{S}(ArrayType{FT}; Nij,Nk,Nv,Nh); test_mapreduce_2!(context, data_view(data)) # TODO: test
    # data = DataLayouts.IH1JH2{S}(ArrayType{FT}; Nij);             test_mapreduce_2!(context, data_view(data)) # TODO: test
end
