...
import ClimaCore.DataLayouts: VIHF, slab, column, VF, IHF, vindex, slab_index
...
@testset "VIHF" begin
    ...
    for FT in TestFloatTypes
        ...
        data = VIHF{S}(ArrayType{FT}, rand; Nv, Ni, Nh)
        array = parent(data)
        sum(x -> x[2], data)
        ...
        @test array[1, 1, 1, 2] == -2.0
        @test array[1, 1, 1, 3] == -3.0
...
@testset "VIHF type safety" begin
    Nv = 1 # number of vertical levels
    ...
    data = VIHF{typeof(SA)}(ArrayType{Float64}, zeros; Nv, Ni, Nh)
...
@testset "broadcasting between VIHF data object + scalars" begin
    FT = Float64
    Nv = 2
    Nh = 2
    S = Complex{Float64}
    data1 = VIHF{S}(ArrayType{FT}, ones; Nv, Ni = 2, Nh = 2)
    res = data1 .+ 1
    @test res isa VIHF{S, Nv}
    @test parent(res) ==
...
@testset "broadcasting between VF + IHF data object => VIHF" begin
    FT = Float64
    S = Complex{FT}
    Nv = 3
    Nh = 2
    data_vf = VF{S}(ArrayType{FT}, ones; Nv)
    data_ihf = IHF{FT}(ArrayType{FT}, ones; Ni = 2, Nh = 2)
    data_vihf = data_vf .+ data_ihf
    @test data_vihf isa VIHF{S, Nv}
    @test size(data_vihf) == (2, 1, 1, 3, 2)
    @test parent(data_vihf) ==
          FT[f == 1 ? 2 : 1 for v in 1:3, i in 1:2, h in 1:2, f in 1:2]

    @test parent(data_vihf .+ data_vf) ==
          FT[f == 1 ? 3 : 2 for v in 1:3, i in 1:2, h in 1:2, f in 1:2]
    @test parent(data_vihf .+ data_ihf) ==
          FT[f == 1 ? 3 : 1 for v in 1:3, i in 1:2, h in 1:2, f in 1:2]

end

@testset "fill" begin
    data = IHF{Float64}(ArrayType{Float64}, ones; Ni = 3, Nh = 3)
    data .= 2.0
