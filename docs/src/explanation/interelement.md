...
Two fluxes are provided for a physical flux function `F`:
  ...
  - `Operators.RusanovNumericalFlux(F, wavespeedfn)`: the central flux plus
    `(λ/2)(y⁻ − y⁺)`, with `λ = max(wavespeedfn(args⁻...), wavespeedfn(args⁺...))`
    an upper bound on the normal signal speed. The penalty acts on the jump
    across the face and is the dissipation
...
model's tendency function then works on both discretizations:
