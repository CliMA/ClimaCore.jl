...
equations, and it runs on CPUs and on NVIDIA GPUs.
...
## What ClimaCore provides
  ...
  - **One tendency for CG and DG.** The discretization is a property of the
    space. Element-local tendencies are completed by direct stiffness summation
    on CG spaces and by interface numerical fluxes on DG spaces, so model code
    can use CG and DG interchangeably
    ([tutorial](tutorials/cg_dg_switch.md)).
  - **Portable performance.** Broadcast expressions over fields compile to CPU
    loops or CUDA kernels. The CliMA atmosphere built on
