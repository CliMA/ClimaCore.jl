...
[Sridhar22a](@cite) set the requirements of portability and of a single model
from the boundary layer to the globe. This page
describes the objects a model is assembled from, the boundaries between
ClimaCore and its neighbors, and the range of configurations it serves.
...
[introduction tutorial](../tutorials/fields_and_operators.md) walks through the
hierarchy with code.
