...
ClimaCore's `ClimaCoreRecipesBaseExt` extension, activated when `Plots` is loaded), and `TerminalLoggers` for progress bars. The extruded
driver also loads `JLD2`. Individual cases add `NCDatasets`, `QuadGK`,
`Colors`, `DocStringExtensions`, or `ProgressLogging`.
...
## Steps
 ...
 1. From the repository root, build a shared environment once with those
    ...
    julia --project=@climacore-examples -e '
        ...
        Pkg.add(["ClimaComms", "ClimaTimeSteppers", "Plots", "TerminalLoggers", "JLD2",
