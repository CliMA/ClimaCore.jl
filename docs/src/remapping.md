...
!!! note "ClimaInterpolations.jl required"
    Using `PressureInterpolator` requires loading the `ClimaInterpolations.jl`
    package.
...
pressure_interp = Remapping.PressureInterpolator(pressure_field, pressure_levels)

# Interpolate the field to pressure coordinates
# This mutates field
field_on_pressure_space = Remapping.interpolate_pressure!(field, pressure_interp)

# Get the pressure field and space with pressure as the vertical
p_field = Remapping.pfull_field(pressure_interp)
p_space = Remapping.pressure_space(pressure_interp)

# If the pressure field change, then you need to call update! once.
Remapping.update!(pressure_intp)

# This mutates both field_on_pressure_space and field and the interpolation done
# in-place
...
1. **Ensure monotonicity**: Applies a cumulative maximum along each column to
   ensure pressure decreases monotonically with height.
2. **Vertical interpolation**: Interpolates field values to the specified
   pressure coordinates using linear interpolation with flat boundary
   conditions.
...
!!! note "Boundary conditions"
    ...
    model's vertical range will be extrapolated using these flat conditions and
    may be inaccurate.
